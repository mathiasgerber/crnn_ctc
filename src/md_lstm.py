...
"""
Source: https://github.com/philipperemy/tensorflow-multi-dimensional-lstm
"""
...
class MultiDimensionalLSTMCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            ...
            i, j, f1, f2, o = tf.split(value=concat, num_or_size_splits=5,
                                       axis=1)
            ...
            new_c = (c1 * tf.nn.sigmoid(f1 + self._forget_bias) +
                     c2 * tf.nn.sigmoid(
                        f2 + self._forget_bias) + tf.nn.sigmoid(i) *
                     self._activation(j))
...
def multi_dimensional_rnn_while_loop(rnn_size, input_data, sh, dims=None,
                                     scope_n="layer1"):
    ...
    with tf.variable_scope("MultiDimensionalLSTMCell-" + scope_n,
        ...
        if X_dim % X_win != 0:
            # Get offset size
            offset = tf.zeros(
                [batch_size_runtime, X_win - (X_dim % X_win), Y_dim, channels])
        ...
        if Y_dim % Y_win != 0:
            # Get offset size
            offset = tf.zeros(
                [batch_size_runtime, X_dim, Y_win - (Y_dim % Y_win), channels])
        ...
        x = tf.reshape(input_data,
                       [batch_size_runtime, h, w, features])
        ...
        inputs_ta = tf.TensorArray(dtype=tf.float32, size=h * w,
                                   name='input_ta')
        ...
        def body(time_, outputs_ta_, states_ta_):
            ...
            current_state = state_up[0], state_last[0], state_up[1], \
                            state_last[1]
        ...
        result, outputs_ta, states_ta = tf.while_loop(condition, body,
                                                      [time, outputs_ta,
                                                       states_ta],
        ...
        y = tf.reshape(outputs,
                       [h, w, batch_size_runtime, rnn_size])
